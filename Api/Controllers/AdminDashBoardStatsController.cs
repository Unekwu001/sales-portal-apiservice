    ...
    public class AdminDashBoardStatsController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetWeeklyResidentialSignups()
        {
            try
            {
                ...
                var currentWeekSignUps = await _dashboardService.GetWeeklyResidentialSignUpCountAsync(currentWeekStart, currentWeekStart.AddDays(7));
                var previousWeekSignUps = await _dashboardService.GetWeeklyResidentialSignUpCountAsync(previousWeekStart, previousWeekStart.AddDays(7));
                ...
                var trendMessage = percentageChange == 100 ? "No previous sign-ups to compare" : $"{percentageChange:+0;-0}% than last week";

                // Handle negative differences
                var signUpsForTheWeek = difference >= 0 ? $"+{difference}" : $"{difference}";

                // If no sign-ups, ensure proper display
                if (currentWeekSignUps == 0)
                {
                    signUpsForTheWeek = "0";
                    trendMessage = "No residential sign-ups this week";
                }

                var result = new ViewWeeklySignUpDto
                {
                    SignUpsForTheWeek = signUpsForTheWeek,
                    Trend = trendMessage
                };
            ...
            {
                _logger.LogError(ex, "An error occurred while trying to view weekly residential sign-ups");
                return BadRequest(ipNXApiResponse.Failure("An error occured while trying to view residential sign up Trends"));
            }
        }
        ...
        public async Task<IActionResult> GetWeeklySmeSignups()
        {
            try
            {
                ...
                var currentWeekSignUps = await _dashboardService.GetWeeklySmeSignUpCountAsync(currentWeekStart, currentWeekStart.AddDays(7));
                var previousWeekSignUps = await _dashboardService.GetWeeklySmeSignUpCountAsync(previousWeekStart, previousWeekStart.AddDays(7));
                ...
                {
                    signUpsForTheWeek = "0";
                    trendMessage = "No sme sign-ups this week";
                }
            ...
            {
                _logger.LogError(ex, "An error occurred while trying to view weekly sme sign-ups");
                return BadRequest(ipNXApiResponse.Failure("An error occured while trying to view sme sign up Trends"));
            }
        }
        ...
        public async Task<IActionResult> GetWeeklyTotalSignups()
        {
            try
            {
                ...
                var currentWeekSignUps = await _dashboardService.GetWeeklyTotalSignUpCountAsync(currentWeekStart, currentWeekStart.AddDays(7));
                var previousWeekSignUps = await _dashboardService.GetWeeklyTotalSignUpCountAsync(previousWeekStart, previousWeekStart.AddDays(7));
                ...
                {
                    signUpsForTheWeek = "0";
                    trendMessage = "No sign-ups this week";
                }

                var result = new ViewWeeklySignUpDto
                {
                    SignUpsForTheWeek = currentWeekSignUps.ToString("N0"),
                    Trend = trendMessage
                };
            ...
            {
                _logger.LogError(ex, "An error occurred while trying to view weekly total sign-ups");
                return BadRequest(ipNXApiResponse.Failure("An error occured while trying to view total sign up Trends"));
            }
        }
