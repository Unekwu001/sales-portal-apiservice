        ...
        public async Task<IActionResult> AddDiscount([FromBody] DiscountDto discountDto) 
        {
            ...
            try
            {
                ...
                foreach (var id in discountDto.PlanTypeIds)
                {
                    ...
                    planTypes.Add(existingPlanType);
                }

                var streetNameExists = await _discountService.StreetNameExistsAsync(discountDto.State,discountDto.Street);
                if (streetNameExists)
                {
                    return Conflict(ipNXApiResponse.Failure("A discount already exists for this street"));
                }
                await _discountService.AddAndSaveDiscountAsync(discountDto, userId, planTypes);
